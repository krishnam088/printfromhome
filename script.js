        ...
        if (imageSlider) {
            imageSlider.innerHTML = files.map((f, fIdx) => {
                ...
                if (f.pageImages && f.pageImages.length > 1) {
                    ...
                    if (singleImgSrc) {
                        // Strict check: Sirf tabhi img tag banega jab wo actual converted image ho (data:image/ ya image type)
                        ...
                        if (isActualImage) {
                            ...
                            // Document files (.doc, .docx, etc.) ke liye clean file icon & name card
                            previewContent = `
